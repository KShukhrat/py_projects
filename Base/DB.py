...
# Cursor yaratish
cur = connect.cursor()
connect.autocommit = True#commit qiladi auto tarzda agarda qilinmasa
                        # pastda keltirilgani yoziladi ohirida
...
def add(name: str, date=None):
    if date is not None:
        cur.execute("INSERT INTO test(name, date) values (%s, %s)",
                    [name, date])  # qovuslar () yoki bu [] ko'rinishda bo'lishi mumkin
    else:
